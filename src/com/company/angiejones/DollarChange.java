package com.company.angiejones;

import java.util.Scanner;

public class DollarChange {
    /**
     * So, you're going to create a program that asks the user to enter the quantities for the following coins.
     *
     * You'll ask them how many pennies would you like?
     * How many nickels would you like?
     * How many dimes?
     * And then, how many quarters?
     * What your program is going to do is count up the value of all the change that they answered.
     *
     * If it's exactly one dollar then they win the game.
     * If it's more than one dollar, then you need to tell them that it's more than one dollar and how much they went over.
     * If it's less than one dollar, tell them how much they went under.
     * Feel free to change this game for your country's currency, so if you don't use dollars and you use something else that's fine.
     *
     * So, for an example of this let's say that I ask the user:
     *
     * How many pennies? - And they said 0.
     * How many nickels? - 0
     * How many dimes? - 0
     * How many quarters? - 4
     * Then my program would know that, okay quarters are worth 25 cents. There's 4 of these, let me multiply those and get the value. Oh! that equals one dollar, so I let them know that they win. So that's an example of the game.
     *
     * Good luck!
     */

    /**
     * To solve this problem understand how many pennies, nickel, dime and quarter make a dollar
     *
     * ANSWER: 100 pennies, 20 nickels, 10 dimes, or 4 quarters; each = 1 dollar.
     */

    public static void main(String[] args) {
        //initialise what we know
        double penny = 0.01;
        double nickel = 0.05;
        double dime = 0.10;
        double quarter = 0.25;
        int dollar = 1;

        // get user inputs
        Scanner scanner = new Scanner(System.in);

        System.out.println("Please Enter your name");
        String name = scanner.nextLine();

        System.out.println("Welcome to 'Change for a Dollar' square. \n" + name + ", let's get started");
        System.out.println("Your goal is to enter enough change to make exactly $1.00");

        System.out.println("How many pennies would you like?");
        int userPenny = scanner.nextInt();
        System.out.println("How many nickels would you like?");
        int userNickel = scanner.nextInt();
        System.out.println("How many dimes would you like?");
        int userDime = scanner.nextInt();
        System.out.println("How many quarters would you like?");
        int userQuarter = scanner.nextInt();

        scanner.close();

        //calculate change
        double total = (penny * userPenny) + (nickel * userNickel) + (dime * userDime) + (quarter * userQuarter);

        //make a decision
        if (total < dollar) {
            double amountShort = dollar - total;
            System.out.println("You lose, your change was short by: " + String.format("%.2f", amountShort) );
        }else if (total > dollar) {
            double amountOver = total - dollar;
            System.out.println("You lose, your change was over by: " + String.format("%.2f", amountOver) );
        } else {
            System.out.println("Congratulations! you won......That's exactly $1.00");
        }



        // make the calculation and take a decision and output result

    }
}
